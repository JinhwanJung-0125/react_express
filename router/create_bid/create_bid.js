...
const eligible_audit_EmptyBid = path.resolve("../", "CMC_Project_EliglbleAudit_Converted-master/AutoBid/EmptyBid")  //적격심사 입찰서 작성 EmptyBid 폴더 위치

/**입찰서 작성과 관련된 router */
export const router = express.Router();

router.post('/revised_test', (req, res, next) => {    //간이종심제 Bid 만들기
    ...
    db.query('select bidPath from emptybid where bidID = ?', [bidName], (err, result, field) => {    //DB로부터 bidName에 대한 서버에 저장되어 있는 공내역서의 path를 조회
        ...
        if (result.length > 0) {  //찾았다면
            ...
            if (!fs.existsSync(folder_path + "\\" + req.session.nickname + "\\" + bidName)) {  //사용자가 만든 입찰서는 사용자 전용 폴더로 따로 관리하기
            ...
            db.query("insert into userBidFiles (userID, bidID, bidPath, balancedRate, personalRate) value (?, ?, ?, ?, ?)", [req.session.nickname, bidName, bidName + "\\" + CompanyName + "_" + bidName + "_업평_" + BalancedRate + "_내사정율_" + PersonalRate + ".BID", BalancedRate, PersonalRate], (err2, result, field) => {
                if (err2) next(err2);
...
router.post('/eligible_audit', (req, res) => {   //적격심사 Bid 만들기
    ...
    db.query('select bidPath from emptybid where bidID = ?', [bidName], (err, result, field) => {    //DB로부터 bidName에 대한 서버에 저장되어 있는 공내역서의 path를 조회
        ...
        if (result.length > 0) {  //찾았다면
            //공내역서를 복사해 작업 폴더로 옳김
            fs.copyFileSync(folder_path + "\\" + result[0].bidPath, eligible_audit_EmptyBid + "\\" + bidName + ".BID");

            //사용자가 입력한 대로 입찰서 작성
            execute_eligible(laborRate, expenseRate, genMngRate, profitRate, difficultyRate, CompanyName, CompanyNum, basePrice, estimateRating);
            ...
            //아까 만든 입찰서를 사용자 전용 폴더에 옳긴다.
            fs.renameSync(eligible_audit_EmptyBid + "\\" + bidName + '.BID', folder_path + "\\" + req.session.nickname + '\\' + bidName + "\\" + CompanyName + "_" + bidName + "_업평_" + BalancedRate + "_내사정율_" + PersonalRate + ".BID");
