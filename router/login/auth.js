...
router.post('/login_process', (req, res) => {   //로그인 프로세스
    ...
    if(username !== undefined && password !== undefined){   
        ...
        db.query('select * from users where id = ?', [username], (err, result, field) => {    //db에 유저 검색
            if(err) err;  //db 오류

            if(result.length > 0){  //결과를 찾았다면

                bcrypt.compare(password, result[0].password, (err, isCorrect) => {   //password가 맞는지 학인
                    if(isCorrect === true){ //password도 맞으면
...
router.get('/authCheck', (req, res) => {   //세션으로 로그인 여부 확인
